...
export default async function Home({ searchParams }) {
	...
	const query = "SELECT * FROM products WHERE title LIKE $1 OR description LIKE $1";
	const values = [`%${searchParams?.search}%`];
	const { rows: searchResults } = await client.query(query, values);

	return (
		<main className='p-4 flex flex-col gap-4'>
			<div className=''>{searchParams?.search ? <SearchInfo searchTerm={searchParams?.search} /> : <SortSelect />}</div>
